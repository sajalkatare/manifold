...
class SupportState(TypedDict):
    ...
    ticket_id: str
    final_response: str
    payment_type: str       # 'prepaid' | 'cod' | 'coupon'
...
@tool
def calculate_shipping_charge(customer_tier: str,payment_type: str) -> float:
    """
    Calculate shipping charges based on customer tier
    and payment type.

    Business Rules:
    - Prepaid orders always have free shipping.
    - Premium customers pay $10 for Cash on Delivery.
    - Standard customers pay $20 for Cash on Delivery.
...
def extract_payment_type(state: SupportState) -> dict:
...
def calculate_shipping(state: SupportState) -> dict:
    charge = calculate_shipping_charge.invoke({
        "customer_tier": state["customer_tier"],
        "payment_type": state["payment_type"],
    })

    print(f"  [shipping] charge = ${charge}")
...
def build_graph():
    ...
    b.add_node("extract_payment_type", extract_payment_type)
    b.add_node("calculate_shipping", calculate_shipping)

    b.set_entry_point("extract_customer_id")
    b.add_edge("extract_customer_id", "lookup_tier")
    b.add_edge("lookup_tier", "extract_payment_type")
    b.add_edge("extract_payment_type", "calculate_shipping")
    b.add_edge("calculate_shipping", "classify_with_llm")
...
if __name__ == "__main__":
    ...
    run(graph, "Hi, my ID is CUST-555. I will pay via COD.",
        "TEST 3: Standard + COD")
    run(graph, "Hi, my ID is CUST-001. I want to place an order with COD.",
        "TEST 4: Premium + COD")