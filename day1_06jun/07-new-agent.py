...
def get_aqi(city: str) -> str:
    """
    Get the current AQI for a given city
    Use this when the user asks about air quality.
    
    Args:
        city: the name of the city (e.g. 'Mumbai', 'Delhi', 'London')
    Returns:
        The current AQI for the given city
    """
    aqi_data = {
        'mumbai': 50,
        'delhi': 100,
    ...
    aqi = aqi_data.get(city.lower(), 'unknown')
    return f"The AQI in {city} is {aqi}"
...
print(r2)
print("--------------------------------")
r3 = agent.invoke({"messages": [HumanMessage(content="What is the AQI in mumbai?")]}) # direct approach
print(r3)
print("--------------------------------")